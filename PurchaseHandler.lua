...
for i,v in pairs(script.Parent.Essentials:GetChildren()) do
	if v.Name == "PartCollector" then
		v.Touched:Connect(function(Part)
			...
			if Part:FindFirstChild('Cash') and AutoCollector == false then
				Money.Value = Money.Value + Part.Cash.Value
				Debris:AddItem(Part,0.1)
			elseif Part:FindFirstChild('Cash') and AutoCollector == true and tycoonOwner ~= nil then
				local Stats = game:GetService("ServerStorage"):WaitForChild("PlayerMoney"):WaitForChild(tycoonOwner.Name)
				if Stats ~= nil then
					Stats.Value += Part.Cash.Value
					Debris:AddItem(Part,0.1)
				end
			end
		end)
